...
public sealed class Parser {
    ...
    public void Parse(ReadOnlySpan<char> str) {
        ...
        while (str.Length > 0) {
            ...
            if (str[index + 1] == '`') {
                ...
                index = str.IndexOf('`');
                if (index == -1)
                    throw new Exception($"missing ending double `+, `- or ``, at {str[..index]}");
                ...
                switch (str[index + 1]) {
                    ...
                    case '`':
                        ...
                        break;
                    default:
                        throw new Exception($"`+, `- or `` expected, only single ` found, at {str[..index]}");
                }

                str = str[(index + 2)..];
                if (str.Length >= Environment.NewLine.Length)
                    if (str[..Environment.NewLine.Length].CompareTo(Environment.NewLine, StringComparison.Ordinal) == 0)
            ...
            // single tick
                ...
                index = str.IndexOf('`');
                if (index == -1)
                    throw new Exception($"missing single `, at {str[..index]}");
    ...
    private void IndentWriting(ReadOnlySpan<char> lines, int indentation) {
        while (true) {
            ...
            int nextPos = lines.IndexOf(Environment.NewLine);
            if (nextPos == -1) {
